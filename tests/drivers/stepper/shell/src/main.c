...
/* Global variables */
static const struct device *const fake_stepper_driver_dev = DEVICE_DT_GET(DT_NODELABEL(fake_stepper_driver));
...
ZTEST(stepper_shell, test_stepper_drv_set_micro_step_res)
{
	...
	ASSERT_STEPPER_FUNC_CALLED(fake_stepper_drv_set_micro_step_res_fake, fake_stepper_driver_dev, err);
...
ZTEST(stepper_shell, test_stepper_drv_get_micro_step_res)
{
	...
	ASSERT_STEPPER_FUNC_CALLED(fake_stepper_drv_get_micro_step_res_fake, fake_stepper_driver_dev, err);
}
