...
enum stepper_motion_control_mode {
	/** Constant speed mode */
	STEPPER_MOTION_CONTROL_MODE_CONSTANT_SPEED = 0,
	/** Ramp mode */
	STEPPER_MOTION_CONTROL_MODE_RAMP = 1,
};

/**
 * @brief Stepper Events
 */
enum stepper_motion_control_event {
	/** Steps set using move_by or move_to have been executed */
	STEPPER_MOTION_CONTROL_EVENT_STEPS_COMPLETED = 0,
	/** Stall detected */
	STEPPER_MOTION_CONTROL_EVENT_STALL_DETECTED = 1,
	/** Left end switch status changes to pressed */
	STEPPER_MOTION_CONTROL_EVENT_LEFT_END_STOP_DETECTED = 2,
	/** Right end switch status changes to pressed */
	STEPPER_MOTION_CONTROL_EVENT_RIGHT_END_STOP_DETECTED = 3,
	/** Stepper has stopped */
	STEPPER_MOTION_CONTROL_EVENT_STOPPED = 4,
};
 ...
 */
typedef int (*stepper_motion_control_move_to_t)(const struct device *dev, const int32_t micro_steps);

/**
 * @brief Move the stepper by a specific number of steps
 */
typedef int (*stepper_motion_control_move_by_t)(const struct device *dev, const int32_t micro_steps);

/**
 * @brief Run the stepper in a specific direction
 */
typedef int (*stepper_motion_control_run_t)(const struct device *dev,
 ...
 */
typedef int (*stepper_motion_control_stop_t)(const struct device *dev);

/**
 * @brief Check if the stepper is currently moving
 */
typedef int (*stepper_motion_control_is_moving_t)(const struct device *dev, bool *is_moving);

/**
 * @brief Set the step interval of the stepper motor
 */
typedef int (*stepper_motion_control_set_step_interval_t)(const struct device *dev,
						   const uint64_t microstep_interval_ns);

/**
 * @brief Get the actual position of the stepper motor
 */
typedef int (*stepper_motion_control_get_actual_position_t)(const struct device *dev, int32_t *value);

/**
 * @brief Set the reference position of the stepper motor
 */
typedef int (*stepper_motion_control_set_reference_position_t)(const struct device *dev,
							const int32_t value);

/**
 * @brief Set the stepper control mode
 */
typedef int (*stepper_motion_control_set_mode_t)(const struct device *dev,
					  const enum stepper_motion_control_mode mode);

/**
 * @brief Callback function for stepper events
 */
typedef void (*stepper_motion_control_event_callback_t)(const struct device *dev,
 ...
 */
typedef int (*stepper_motion_control_set_event_callback_t)(const struct device *dev,
...
__subsystem struct stepper_motion_control_driver_api {
	...
	stepper_motion_control_set_step_interval_t set_step_interval;
	...
	stepper_motion_control_set_mode_t set_mode;
 ...
 * @retval 0 Success
 */
__syscall int stepper_motion_control_move_to(const struct device *dev, const int32_t micro_steps);

static inline int z_impl_stepper_motion_control_move_to(const struct device *dev,
						 const int32_t micro_steps)
{
	const struct stepper_motion_control_driver_api *api = dev->api;
 ...
 * @retval 0 Success
 */
__syscall int stepper_motion_control_move_by(const struct device *dev, const int32_t micro_steps);

static inline int z_impl_stepper_motion_control_move_by(const struct device *dev,
						 const int32_t micro_steps)
{
	const struct stepper_motion_control_driver_api *api = dev->api;
 ...
 * @retval 0 Success
 */
__syscall int stepper_motion_control_run(const struct device *dev, const enum stepper_direction direction);

static inline int z_impl_stepper_motion_control_run(const struct device *dev,
					     const enum stepper_direction direction)
{
	const struct stepper_motion_control_driver_api *api = dev->api;
 ...
 * @retval 0 Success
 */
__syscall int stepper_motion_control_stop(const struct device *dev);

static inline int z_impl_stepper_motion_control_stop(const struct device *dev)
{
	const struct stepper_motion_control_driver_api *api = dev->api;
 ...
 * @retval 0 Success
 */
__syscall int stepper_motion_control_set_step_interval(const struct device *dev,
						const uint64_t microstep_interval_ns);
static inline int z_impl_stepper_motion_control_set_step_interval(const struct device *dev,
							   const uint64_t microstep_interval_ns)
{
	const struct stepper_motion_control_driver_api *api = dev->api;
 ...
 * @retval 0 Success
 */
__syscall int stepper_motion_control_get_actual_position(const struct device *dev, int32_t *value);
static inline int z_impl_stepper_motion_control_get_actual_position(const struct device *dev,
							     int32_t *value)
{
	const struct stepper_motion_control_driver_api *api = dev->api;
 ...
 * @retval 0 Success
 */
__syscall int stepper_motion_control_set_reference_position(const struct device *dev, const int32_t value);

static inline int z_impl_stepper_motion_control_set_reference_position(const struct device *dev,
								const int32_t value)
{
	const struct stepper_motion_control_driver_api *api = dev->api;
...
__syscall int stepper_motion_control_set_mode(const struct device *dev,
				       const enum stepper_motion_control_mode mode);
static inline int z_impl_stepper_motion_control_set_mode(const struct device *dev,
						  const enum stepper_motion_control_mode mode)
{
	const struct stepper_motion_control_driver_api *api = dev->api;
 ...
 * @retval 0 Success
 */
__syscall int stepper_motion_control_is_moving(const struct device *dev, bool *is_moving);

static inline int z_impl_stepper_motion_control_is_moving(const struct device *dev, bool *is_moving)
{
	const struct stepper_motion_control_driver_api *api = dev->api;
 ...
 * @retval 0 Success
 */
__syscall int stepper_motion_control_set_event_callback(const struct device *dev,
						 stepper_motion_control_event_callback_t callback,
						 void *user_data);

static inline int z_impl_stepper_motion_control_set_event_callback(
