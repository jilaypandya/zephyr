...
void stepper_callback(const struct device *dev, const enum stepper_control_event event, void *user_data)
{
	switch (event) {
...
int main(void)
{
	...
	int32_t tmc_velocity = DT_PROP(DT_ALIAS(stepper_controller), vmax) * CONFIG_MAX_VELOCITY_MULTIPLIER;
	(void)tmc50xx_stepper_set_max_velocity(stepper_control, tmc_velocity);
