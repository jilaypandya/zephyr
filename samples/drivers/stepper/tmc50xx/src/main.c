...
void stepper_callback(const struct device *dev, const uint8_t stepper_index,const enum stepper_event event, void *user_data)
{
...
int main(void)
{
	...
	for (;;) {
		if (k_sem_take(&steps_completed_sem, K_FOREVER) == 0) {
			...
			if (stepper_get_actual_position(stepper_controller, stepper_index, &actual_position) == 0) {
