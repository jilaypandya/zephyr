...
const struct device *stepper_control = DEVICE_DT_GET(DT_ALIAS(stepper_motion_controller));
...
void stepper_callback(const struct device *dev, const enum stepper_motion_control_event event,
		      void *user_data)
{
	switch (event) {
...
int main(void)
{
	...
	stepper_motion_control_set_event_callback(stepper_control, stepper_callback, NULL);

	LOG_DBG("Enabling stepper driver %s", stepper->name);
	stepper_enable(stepper);

	LOG_DBG("Setting reference position to 0 for %s", stepper_control->name);
	stepper_motion_control_set_reference_position(stepper_control, 0);

	LOG_DBG("Moving to %d", ping_pong_target_position);
	stepper_motion_control_move_by(stepper_control, ping_pong_target_position);

	/* Change Max Velocity during runtime */
	int32_t tmc_velocity =
		DT_PROP(DT_ALIAS(stepper_motion_controller), vmax) * CONFIG_MAX_VELOCITY_MULTIPLIER;
	(void)tmc50xx_stepper_motion_control_set_max_velocity(stepper_control, tmc_velocity);
