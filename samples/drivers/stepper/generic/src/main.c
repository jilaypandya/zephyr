...
/* Every motion controller must support atleast one stepper */
...
static void stepper_callback(const struct device *dev, const uint8_t stepper_index, const enum stepper_event event,
			     void *user_data)
{
	switch (event) {
...
int main(void)
{
	...
	stepper_set_event_callback(stepper_motion_controller,STEPPER_INDEX, stepper_callback, NULL);
	stepper_set_reference_position(stepper_motion_controller,STEPPER_INDEX, 0);
	stepper_set_microstep_interval(stepper_motion_controller, STEPPER_INDEX,CONFIG_STEP_INTERVAL_NS);
	...
	for (;;) {
		...
		case STEPPER_MODE_ROTATE_CW:
			stepper_run(stepper_motion_controller, STEPPER_INDEX,  STEPPER_DIRECTION_POSITIVE);
			LOG_INF("mode: rotate cw\n");
			break;
		case STEPPER_MODE_ROTATE_CCW:
			stepper_run(stepper_motion_controller, STEPPER_INDEX, STEPPER_DIRECTION_NEGATIVE);
			LOG_INF("mode: rotate ccw\n");
			break;
		case STEPPER_MODE_PING_PONG_RELATIVE:
			ping_pong_target_position *= -1;
			stepper_move_by(stepper_motion_controller, STEPPER_INDEX,ping_pong_target_position);
			LOG_INF("mode: ping pong relative\n");
			break;
		case STEPPER_MODE_PING_PONG_ABSOLUTE:
			ping_pong_target_position *= -1;
			stepper_move_to(stepper_motion_controller,STEPPER_INDEX, ping_pong_target_position);
...
static void monitor_thread(void)
{
	for (;;) {
		...
		stepper_get_actual_position(stepper_motion_controller, STEPPER_INDEX, &actual_position);
