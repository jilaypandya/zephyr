...
#include "stepper_motion_control_timing_source.h"

#include <zephyr/logging/log.h>
LOG_MODULE_REGISTER(stepper_motion_control, CONFIG_STEPPER_CONTROL_LOG_LEVEL);
...
void stepper_trigger_callback(const struct device *dev, enum stepper_motion_control_event event)
{
...
static void stepper_work_event_handler(struct k_work *work)
{
	...
	enum stepper_motion_control_event event;
	int ret;
...
static int z_stepper_motion_control_move_by(const struct device *dev, int32_t micro_steps)
{
	...
	K_SPINLOCK(&data->lock) {
		...
		stepper_set_direction(config->stepper, data->direction);
...
static int z_stepper_motion_control_stop(const struct device *dev)
{
	...
	stepper_trigger_callback(dev, STEPPER_MOTION_CONTROL_EVENT_STOPPED);
...
static int z_stepper_motion_control_is_moving(const struct device *dev, bool *is_moving)
{
	...
	*is_moving = config->timing_source->is_running(&data->timing_data);
...
static void position_mode_task(const struct device *dev)
{
	...
	if (config->timing_source->needs_reschedule(dev) && data->step_count != 0) {
		...
		stepper_trigger_callback(data->dev, STEPPER_MOTION_CONTROL_EVENT_STEPS_COMPLETED);
...
static DEVICE_API(stepper_motion_control, stepper_motion_control_api) = {
	.move_to = z_stepper_motion_control_move_to,
	.move_by = z_stepper_motion_control_move_by,
	.run = z_stepper_motion_control_run,
	.set_step_interval = z_stepper_motion_control_set_step_interval,
