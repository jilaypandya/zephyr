...
struct stepper_control_data {
	...
	uint8_t event_msgq_buffer[CONFIG_ZEPHYR_STEPPER_MOTION_CONTROL_EVENT_QUEUE_LEN *
				  sizeof(enum stepper_control_event)];
#endif /* CONFIG_ZEPHYR_STEPPER_MOTION_CONTROL_GENERATE_ISR_SAFE_EVENTS */
};

void stepper_trigger_callback(const struct device *dev, enum stepper_control_event event)
{
...
static void stepper_work_event_handler(struct k_work *work)
{
	struct stepper_control_data *data =
		CONTAINER_OF(work, struct stepper_control_data, event_callback_work);
	enum stepper_control_event event;
	int ret;
...
static int z_stepper_control_stop(const struct device *dev)
{
	...
	stepper_trigger_callback(dev, STEPPER_CONTROL_EVENT_STOPPED);
...
static void position_mode_task(const struct device *dev)
{
	...
	if (config->timing_source->needs_reschedule(dev) && data->step_count != 0) {
		...
		stepper_trigger_callback(data->dev, STEPPER_CONTROL_EVENT_STEPS_COMPLETED);
...
static int stepper_control_init(const struct device *dev)
{
	...
	k_msgq_init(&data->event_msgq, data->event_msgq_buffer, sizeof(enum stepper_control_event),
