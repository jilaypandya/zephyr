...
static void execute_callback(const struct device *dev, const enum stepper_motion_control_event event)
{
...
static int tmc50xx_stepper_control_move_to(const struct device *dev, const int32_t micro_steps)
{
	...
	err = stepper_controller_write(config->controller, TMC50XX_XTARGET(config->index), micro_steps);
	if (err != 0) {
		return -EIO;
	}
							  ...
							  const int32_t position)
{
	...
	err = stepper_controller_write(config->controller, TMC50XX_XACTUAL(config->index), position);
	if (err != 0) {
		return -EIO;
	}
...
static int tmc50xx_stepper_control_is_moving(const struct device *dev, bool *is_moving)
{
	...
	err = stepper_controller_read(config->controller, TMC50XX_DRVSTATUS(config->index), &reg_value);
...
{
	if (mode == STEPPER_MOTION_CONTROL_MODE_CONSTANT_SPEED) {
		return -ENOTSUP;
	}
	return 0;
}
...
int tmc50xx_stepper_control_set_max_velocity(const struct device *dev, uint32_t velocity)
{
	...
	err = stepper_controller_write(config->controller, TMC50XX_VMAX(config->index), velocity_fclk);
	if (err != 0) {
				     ...
				     const struct tmc_ramp_generator_data *ramp_data)
{
	...
	err = stepper_controller_write(config->controller, TMC50XX_VSTART(config->index), ramp_data->vstart);
	if (err != 0) {
		return -EIO;
	}
	err = stepper_controller_write(config->controller, TMC50XX_A1(config->index), ramp_data->a1);
	if (err != 0) {
		return -EIO;
	}
	err = stepper_controller_write(config->controller, TMC50XX_AMAX(config->index), ramp_data->amax);
	if (err != 0) {
		return -EIO;
	}
	err = stepper_controller_write(config->controller, TMC50XX_D1(config->index), ramp_data->d1);
	if (err != 0) {
		return -EIO;
	}
	err = stepper_controller_write(config->controller, TMC50XX_DMAX(config->index), ramp_data->dmax);
	if (err != 0) {
		return -EIO;
	}
	err = stepper_controller_write(config->controller, TMC50XX_V1(config->index), ramp_data->v1);
	if (err != 0) {
		return -EIO;
	}
	err = stepper_controller_write(config->controller, TMC50XX_VMAX(config->index), ramp_data->vmax);
	if (err != 0) {
		return -EIO;
	}
	err = stepper_controller_write(config->controller, TMC50XX_VSTOP(config->index), ramp_data->vstop);
	if (err != 0) {
		return -EIO;
	}
	...
	err = stepper_controller_write(config->controller, TMC50XX_VHIGH(config->index), ramp_data->vhigh);
	if (err != 0) {
		return -EIO;
	}
...
static DEVICE_API(stepper_motion_control, tmc50xx_stepper_control_api) = {
