...
struct tmc50xx_stepper_control_data {
	...
	stepper_motion_control_event_callback_t callback;
...
static int tmc50xx_stepper_control_set_event_callback(
	const struct device *dev, stepper_motion_control_event_callback_t callback, void *user_data)
{
...
static int read_vactual(const struct tmc50xx_stepper_control_config *config,
			int32_t *actual_velocity)
{
	int err;

	err = stepper_controller_read(config->controller, TMC50XX_VACTUAL(config->index),
				      actual_velocity);
	if (err) {
...
static int stallguard_enable(const struct device *dev, const bool enable)
{
	...
	err = stepper_controller_read(config->controller, TMC50XX_SWMODE(config->index),
				      &reg_value);
	if (err) {
	...
	err = stepper_controller_write(config->controller, TMC50XX_SWMODE(config->index),
				       reg_value);
	if (err) {
...
static void stallguard_work_handler(struct k_work *work)
{
	...
	struct tmc50xx_stepper_control_data *stepper_control_data =
		CONTAINER_OF(dwork, struct tmc50xx_stepper_control_data, stallguard_dwork);
	int err;
	const struct tmc50xx_stepper_control_config *stepper_control_config =
		stepper_control_data->motion_controller->config;

	err = stallguard_enable(stepper_control_data->motion_controller, true);
	if (err == -EAGAIN) {
		k_work_reschedule(dwork,
				  K_MSEC(stepper_control_config->sg_velocity_check_interval_ms));
	}
	if (err == -EIO) {
...
static void execute_callback(const struct device *dev,
...
#ifdef CONFIG_ADI_TMC50XX_RAMPSTAT_POLL_STALLGUARD_LOG

static void log_stallguard(struct tmc50xx_stepper_control_data *stepper_control_data,
			   const uint32_t drv_status)
{
	const struct tmc50xx_stepper_control_config *stepper_control_config =
		stepper_control_data->motion_controller->config;
	int32_t position;
	...
	err = read_actual_position(stepper_control_config, &position);
	if (err != 0) {
		LOG_ERR("%s: Failed to read XACTUAL register",
			stepper_control_data->motion_controller->name);
		return;
	}
	...
	LOG_DBG("%s position: %d | sg result: %3d status: %d",
		stepper_control_data->motion_controller->name, position, sg_result, sg_status);
}
...
static void rampstat_work_reschedule(struct k_work_delayable *rampstat_callback_dwork)
{
	k_work_reschedule(rampstat_callback_dwork,
			  K_MSEC(CONFIG_ADI_TMC50XX_RAMPSTAT_POLL_INTERVAL_IN_MSEC));
}
...
static void rampstat_work_handler(struct k_work *work)
{
	...
	err = stepper_controller_read(stepper_control_config->controller,
				      TMC50XX_DRVSTATUS(stepper_control_config->index),
				      &drv_status);
	if (err != 0) {
		LOG_ERR("%s: Failed to read DRVSTATUS register",
			stepper_control_data->motion_controller->name);
		return;
	}
#ifdef CONFIG_ADI_TMC50XX_RAMPSTAT_POLL_STALLGUARD_LOG
	log_stallguard(stepper_control_data, drv_status);
#endif
	if (FIELD_GET(TMC5XXX_DRV_STATUS_SG_STATUS_MASK, drv_status) == 1U) {
		LOG_INF("%s: Stall detected", stepper_control_data->motion_controller->name);
		err = stepper_controller_write(stepper_control_config->controller,
					       TMC50XX_RAMPMODE(stepper_control_config->index),
					       TMC5XXX_RAMPMODE_HOLD_MODE);
		if (err != 0) {
			LOG_ERR("%s: Failed to stop motor",
				stepper_control_data->motion_controller->name);
			return;
		}
	}
	...
	err = stepper_controller_read(stepper_control_config->controller,
				      TMC50XX_RAMPSTAT(stepper_control_config->index),
				      &rampstat_value);
	if (err != 0) {
		LOG_ERR("%s: Failed to read RAMPSTAT register",
			stepper_control_data->motion_controller->name);
		return;
	}
	...
	if (ramp_stat_values > 0) {
		...
		case TMC5XXX_STOP_LEFT_EVENT:
			LOG_DBG("RAMPSTAT %s:Left end-stop detected",
				stepper_control_data->motion_controller->name);
			execute_callback(stepper_control_data->motion_controller,
					 STEPPER_MOTION_CONTROL_EVENT_LEFT_END_STOP_DETECTED);
			break;
		...
		case TMC5XXX_STOP_RIGHT_EVENT:
			LOG_DBG("RAMPSTAT %s:Right end-stop detected",
				stepper_control_data->motion_controller->name);
			execute_callback(stepper_control_data->motion_controller,
					 STEPPER_MOTION_CONTROL_EVENT_RIGHT_END_STOP_DETECTED);
			break;

		case TMC5XXX_POS_REACHED_EVENT:
			LOG_DBG("RAMPSTAT %s:Position reached",
				stepper_control_data->motion_controller->name);
			execute_callback(stepper_control_data->motion_controller,
					 STEPPER_MOTION_CONTROL_EVENT_STEPS_COMPLETED);
			break;

		case TMC5XXX_STOP_SG_EVENT:
			LOG_DBG("RAMPSTAT %s:Stall detected",
				stepper_control_data->motion_controller->name);
			stallguard_enable(stepper_control_data->motion_controller, false);
			execute_callback(stepper_control_data->motion_controller,
					 STEPPER_MOTION_CONTROL_EVENT_STALL_DETECTED);
			break;
		default:
...
static int tmc50xx_stepper_control_is_moving(const struct device *dev, bool *is_moving)
{
	...
	err = stepper_controller_read(config->controller, TMC50XX_DRVSTATUS(config->index),
				      &reg_value);
...
int tmc50xx_stepper_motion_control_set_max_velocity(const struct device *dev, uint32_t velocity)
{
	const struct tmc50xx_stepper_control_config *config = dev->config;
	...
	err = stepper_controller_write(config->controller, TMC50XX_VMAX(config->index),
				       velocity_fclk);
	if (err != 0) {
							  ...
							  const int32_t position)
{
	...
	err = stepper_controller_write(config->controller, TMC50XX_XACTUAL(config->index),
				       position);
	if (err != 0) {
		return -EIO;
	}
				...
				int32_t *position)
{
	...
	err = stepper_controller_read(config->controller, TMC50XX_XACTUAL(config->index), position);
	if (err != 0) {
		return -EIO;
	}
	return 0;
...
static int tmc50xx_stepper_control_move_to(const struct device *dev, const int32_t micro_steps)
{
	...
	err = stepper_controller_write(config->controller, TMC50XX_XTARGET(config->index),
				       micro_steps);
	if (err != 0) {
		return -EIO;
	}
...
static int tmc50xx_stepper_control_move_by(const struct device *dev, const int32_t micro_steps)
{
	...
	err = stepper_motion_control_get_actual_position(dev, &position);
	if (err != 0) {
		return -EIO;
	}
			     ...
			     const struct tmc_ramp_generator_data *ramp_data)
{
	...
	err = stepper_controller_write(config->controller, TMC50XX_VSTART(config->index),
				       ramp_data->vstart);
	if (err != 0) {
		return -EIO;
	}
	err = stepper_controller_write(config->controller, TMC50XX_A1(config->index),
				       ramp_data->a1);
	if (err != 0) {
		return -EIO;
	}
	err = stepper_controller_write(config->controller, TMC50XX_AMAX(config->index),
				       ramp_data->amax);
	if (err != 0) {
		return -EIO;
	}
	err = stepper_controller_write(config->controller, TMC50XX_D1(config->index),
				       ramp_data->d1);
	if (err != 0) {
		return -EIO;
	}
	err = stepper_controller_write(config->controller, TMC50XX_DMAX(config->index),
				       ramp_data->dmax);
	if (err != 0) {
		return -EIO;
	}
	err = stepper_controller_write(config->controller, TMC50XX_V1(config->index),
				       ramp_data->v1);
	if (err != 0) {
		return -EIO;
	}
	...
	err = stepper_controller_write(config->controller, TMC50XX_VSTOP(config->index),
				       ramp_data->vstop);
	if (err != 0) {
		return -EIO;
	}
	err = stepper_controller_write(config->controller, TMC50XX_TZEROWAIT(config->index),
				       ramp_data->tzerowait);
	if (err != 0) {
		return -EIO;
	}
	err = stepper_controller_write(config->controller, TMC50XX_VHIGH(config->index),
				       ramp_data->vhigh);
	if (err != 0) {
		return -EIO;
	}
	err = stepper_controller_write(config->controller, TMC50XX_VCOOLTHRS(config->index),
				       ramp_data->vcoolthrs);
	if (err != 0) {
		return -EIO;
	}
	err = stepper_controller_write(config->controller, TMC50XX_IHOLD_IRUN(config->index),
				       ramp_data->iholdrun);
	if (err != 0) {
		return -EIO;
	}
...
static int tmc50xx_init(const struct device *dev)
{
	...
	err = stepper_controller_write(dev, TMC5XXX_GCONF, config->gconf);
	if (err != 0) {
		return -EIO;
	}
	...
	err = stepper_controller_read(dev, TMC5XXX_GSTAT, &gstat_value);
	if (err != 0) {
		return -EIO;
	}
...
{
	const struct tmc50xx_stepper_control_config *stepper_control_config = dev->config;
	struct tmc50xx_stepper_control_data *data = dev->data;
	int err;

	LOG_DBG("Controller: %s, Stepper: %s", stepper_control_config->controller->name, dev->name);

	if (stepper_control_config->is_sg_enabled) {
		k_work_init_delayable(&data->stallguard_dwork, stallguard_work_handler);

		err = stepper_controller_write(stepper_control_config->controller,
					       TMC50XX_SWMODE(stepper_control_config->index),
					       BIT(10));
		if (err != 0) {
			return -EIO;
		}

		LOG_DBG("Setting stall guard to %d with delay %d ms",
			stepper_control_config->sg_threshold,
			stepper_control_config->sg_velocity_check_interval_ms);
		if (!IN_RANGE(stepper_control_config->sg_threshold, TMC5XXX_SG_MIN_VALUE,
			      TMC5XXX_SG_MAX_VALUE)) {
			LOG_ERR("Stallguard threshold out of range");
			return -EINVAL;
		}

		int32_t stall_guard_threshold = (int32_t)stepper_control_config->sg_threshold;

		err = stepper_controller_write(
			stepper_control_config->controller,
			TMC50XX_COOLCONF(stepper_control_config->index),
...
#ifdef CONFIG_STEPPER_ADI_TMC50XX_RAMP_GEN
	err = tmc50xx_stepper_set_ramp(dev, &stepper_control_config->default_ramp_config);
	if (err != 0) {
		return -EIO;
	}
#endif
...
static DEVICE_API(stepper_motion_control, tmc50xx_stepper_api) = {
...
static DEVICE_API(stepper_controller, tmc50xx_api) = {
	.write = tmc50xx_write,
	.read = tmc50xx_read,
};

#define TMC50XX_STEPPER_CONTROL_CONFIG_DEFINE(child)                                               \
	...
	static const struct tmc50xx_stepper_control_config                                         \
		tmc50xx_stepper_control_config_##child = {                                         \
...
#define TMC50XX_STEPPER_CONTROL_DATA_DEFINE(child)                                                 \
	static struct tmc50xx_stepper_control_data tmc50xx_stepper_control_data_##child = {        \
		.motion_controller = DEVICE_DT_GET(child),                                         \
	};

#define TMC50XX_STEPPER_DEFINE(child)                                                              \
	DEVICE_DT_DEFINE(child, tmc50xx_stepper_init, NULL, &tmc50xx_stepper_control_data_##child, \
			 &tmc50xx_stepper_control_config_##child, POST_KERNEL,                     \
...
#define TMC50XX_DEFINE(inst)                                                                       \
	...
	DT_INST_FOREACH_CHILD(inst, TMC50XX_STEPPER_CONTROL_CONFIG_DEFINE);                        \
	DT_INST_FOREACH_CHILD(inst, TMC50XX_STEPPER_CONTROL_DATA_DEFINE);                          \
