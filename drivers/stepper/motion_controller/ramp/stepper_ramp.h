 ...
 * @param profile Pointer to the movement specifications
 * @param step_count Number of steps to run.e
 *
 * @return The initial step interval in nanoseconds for the movement
 */
typedef uint64_t (*stepper_ramp_prepare_move_t)(struct avr446_ramp_data *ramp,
                                                const struct avr446_ramp_profile *profile,
                                                uint32_t step_count);

typedef uint64_t (*stepper_ramp_prepare_stop_t)(struct avr446_ramp_data *ramp,
                                                const struct avr446_ramp_profile *profile);

typedef uint64_t (*stepper_ramp_get_next_interval_t)(struct avr446_ramp_data *ramp);
...
#define RAMP_DT_INST_SPEC_GET_API(inst)							\
	(COND_CODE_1(									\
		DT_INST_ENUM_HAS_VALUE(inst, ramp_type, STEPPER_RAMP_TYPE_TRAPEZOIDAL),	\
		(&trapezoidal_ramp_api),						\
		(NULL)))
