...
struct stepper_timing_source_work_cfg
{
};

#define STEPPER_TIMING_SOURCE_DT_INST_WORK_CFG_DEFINE(inst)				\
	static const struct stepper_timing_source_work_cfg stepper_timing_source_cfg_##inst = {};
...
#define STEPPER_TIMING_SOURCE_DT_INST_DEFINE(inst)				\
	...
	COND_CODE_1(STEPPER_TIMING_DT_INST_USE_COUNTER(inst),			\
		...
		(STEPPER_TIMING_SOURCE_DT_INST_WORK_CFG_DEFINE(inst));		\
	)									\
	COND_CODE_1(STEPPER_TIMING_DT_INST_USE_COUNTER(inst),			\
	...
	static struct stepper_timing_source stepper_timing_source_##inst = {	\
		.config = &stepper_timing_source_cfg_##inst,			\
...
// TODO: switch between counter and worker apis
 ...
 */
typedef int (*stepper_timing_source_init)(const struct stepper_timing_source *timing_source);

/**
 * @brief Start the stepper timing source.
 *
 * @param timing_source Pointer to the structure containing timing source config and data.
 * @param interval time interval in nanoseconds after which the callback function should be triggered.
 *
 * @return 0 on success, or a negative error code on failure.
 */
...
// TODO: rename api variables matching naming scheme
