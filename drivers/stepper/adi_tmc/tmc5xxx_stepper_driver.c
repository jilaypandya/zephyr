...
static int read_vactual(const struct tmc5xxx_stepper_config *config, int32_t *actual_velocity)
{
	...
	err = stepper_controller_read(config->controller, TMC50XX_VACTUAL(config->index), actual_velocity);
	if (err) {
...
int tmc5xxx_stallguard_enable(const struct device *dev, const bool enable)
{
	...
	err = stepper_controller_read(config->controller, TMC50XX_SWMODE(config->index), &reg_value);
	if (err) {
	...
	err = stepper_controller_write(config->controller, TMC50XX_SWMODE(config->index), reg_value);
	if (err) {
...
static int tmc50xx_stepper_enable(const struct device *dev)
{
	...
	err = stepper_controller_read(config->controller, TMC50XX_CHOPCONF(config->index), &reg_value);
	if (err != 0) {
		return -EIO;
	}
	...
	return stepper_controller_write(config->controller, TMC50XX_CHOPCONF(config->index), reg_value);
}
					      ...
					      enum stepper_micro_step_resolution res)
{
	...
	err = stepper_controller_write(config->controller, TMC50XX_CHOPCONF(config->index), reg_value);
	if (err != 0) {
		return -EIO;
	}
...
static int tmc50xx_stepper_init(const struct device *dev)
{
	...
	if (stepper_config->is_sg_enabled) {
		...
		err = stepper_controller_write(stepper_config->controller,
