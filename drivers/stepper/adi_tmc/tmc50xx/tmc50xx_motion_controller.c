...
static void stallguard_work_handler(struct k_work *work)
{
	...
	err = tmc50xx_motion_controller_stallguard_enable(data->dev, true);
	if (err == -EAGAIN) {
...
int tmc50xx_motion_controller_stallguard_enable(const struct device *dev, const bool enable)
{
	const struct tmc50xx_motion_controller_config *config = dev->config;
	uint32_t reg_value;
	int err;
...
static int tmc50xx_motion_controller_set_event_callback(const struct device *dev,
							stepper_event_callback_t callback,
							void *user_data)
{
	struct tmc50xx_motion_controller_data *data = dev->data;
...
static int tmc50xx_stepper_move_to(const struct device *dev, const int32_t micro_steps)
{
	...
	if (config->is_sg_enabled) {
		tmc50xx_motion_controller_stallguard_enable(dev, false);
	}
...
static int tmc50xx_stepper_run(const struct device *dev, const enum stepper_direction direction)
{
	...
	if (config->is_sg_enabled) {
		err = tmc50xx_motion_controller_stallguard_enable(dev, false);
		if (err != 0) {
			return -EIO;
		}
	}
...
static DEVICE_API(stepper, tmc50xx_stepper_api) = {
	...
	.set_event_callback = tmc50xx_motion_controller_set_event_callback,
};
