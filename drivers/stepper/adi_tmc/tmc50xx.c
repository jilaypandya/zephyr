...
static int read_vactual(const struct device *controller, const uint8_t stepper_index, int32_t *actual_velocity)
{
...
static int stallguard_enable(const struct device *controller, const uint8_t stepper_index, const bool enable)
{
	const struct tmc50xx_config *tmc50xx_config = controller->config;
	const struct tmc50xx_stepper_config *config = tmc50xx_config->steppers[stepper_index]->config;
	uint32_t reg_value;
	int err;
...
static void execute_callback(const struct device *controller, const uint8_t stepper_index, const enum stepper_event event)
{
	const struct tmc50xx_config *tmc50xx_config = controller->config;
...
static void rampstat_work_handler(struct k_work *work)
{
	...
	if (ramp_stat_values > 0) {
		...
		case TMC5XXX_POS_REACHED_AND_EVENT:
			...
			execute_callback(stepper_config->controller, stepper_config->index, STEPPER_EVENT_STEPS_COMPLETED);
			break;
		...
		case TMC5XXX_STOP_SG_EVENT:
			...
			execute_callback(stepper_config->controller, stepper_config->index, STEPPER_EVENT_STALL_DETECTED);
			break;
		default:
...
static int tmc50xx_stepper_is_moving(const struct device *controller, const uint8_t stepper_index, bool *is_moving)
{
	uint32_t reg_value;
	int err;
...
int tmc50xx_stepper_set_max_velocity(const struct device *controller, const uint8_t stepper_index, uint32_t velocity)
{
	const struct tmc50xx_config *tmc50xx_config = controller->config;
...
static int tmc50xx_stepper_set_reference_position(const struct device *controller, const uint8_t stepper_index, const int32_t position)
{
	...
	LOG_DBG("Stepper motor controller %s set actual position to %d", controller->name, position);
	return 0;
}
...
static int tmc50xx_stepper_get_actual_position(const struct device *controller, const uint8_t stepper_index, int32_t *position)
{
	int err;
...
static int tmc50xx_stepper_move_to(const struct device *controller, const uint8_t stepper_index, const int32_t micro_steps)
{
	...
	const struct tmc50xx_stepper_config *config = controller_config->steppers[0]->config;
	struct tmc50xx_stepper_data *data = controller_config->steppers[0]->data;
	int err;
...
static int tmc50xx_stepper_move_by(const struct device *controller, const uint8_t stepper_index, const int32_t micro_steps)
{
	int err;
	int32_t position;
...
static int tmc50xx_stepper_run(const struct device *controller, const uint8_t stepper_index, const enum stepper_direction direction)
{
	...
	const struct tmc50xx_config *tmc50xx_config = controller->config;
	const struct tmc50xx_stepper_config *config = tmc50xx_config->steppers[0]->config;
	struct tmc50xx_stepper_data *data = tmc50xx_config->steppers[0]->data;
	int err;
			     ...
			     const struct tmc_ramp_generator_data *ramp_data)
{
	...
	err = tmc50xx_write(controller, TMC50XX_TZEROWAIT(stepper_index),
			    ramp_data->tzerowait);
	if (err != 0) {
		return -EIO;
	}
	...
	err = tmc50xx_write(controller, TMC50XX_VCOOLTHRS(stepper_index),
			    ramp_data->vcoolthrs);
	if (err != 0) {
		return -EIO;
	}
	err = tmc50xx_write(controller, TMC50XX_IHOLD_IRUN(stepper_index),
			    ramp_data->iholdrun);
	if (err != 0) {
		return -EIO;
	}
...
#define TMC50XX_DEFINE(inst)									\
	...
	static const struct tmc50xx_config tmc50xx_config_##inst = {				\
		...
		.steppers = (const struct device *[]) {						\
			DT_INST_FOREACH_CHILD(inst, TMC50XX_STEPPER_PTRS)},			\
