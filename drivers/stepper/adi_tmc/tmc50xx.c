...
static void log_stallguard(struct tmc50xx_motion_controller_data *data, const uint32_t drv_status)
{
	...
	LOG_DBG("%s position: %d | sg result: %3d status: %d", data->dev->name, position, sg_result,
		sg_status);
}
...
static void rampstat_work_handler(struct k_work *work)
{
	...
	data->next_index = 1 - data->next_index;
	uint32_t drv_status;
	int err;

	err = tmc50xx_read(dev, TMC50XX_DRVSTATUS(data->next_index), &drv_status);
	if (err != 0) {
	...
	if (FIELD_GET(TMC5XXX_DRV_STATUS_SG_STATUS_MASK, drv_status) == 1U) {
		...
		err = tmc50xx_write(dev, TMC50XX_RAMPMODE(data->next_index),
	...
	err = tmc50xx_read(dev, TMC50XX_RAMPSTAT(data->next_index), &rampstat_value);
	if (err != 0) {
	...
	if (ramp_stat_values > 0) {
		...
		case TMC5XXX_STOP_LEFT_EVENT:
			...
			motion_controller_trigger_cb(config->motion_controllers[data->next_index], STEPPER_EVENT_LEFT_END_STOP_DETECTED);
			break;

		case TMC5XXX_STOP_RIGHT_EVENT:
			LOG_DBG("RAMPSTAT %s:Right end-stop detected", data->dev->name);
			motion_controller_trigger_cb(config->motion_controllers[data->next_index], STEPPER_EVENT_RIGHT_END_STOP_DETECTED);
			break;
		...
		case TMC5XXX_POS_REACHED_AND_EVENT:
			...
			motion_controller_trigger_cb(config->motion_controllers[data->next_index], STEPPER_EVENT_STEPS_COMPLETED);
			break;

		case TMC5XXX_STOP_SG_EVENT:
			LOG_DBG("RAMPSTAT %s:Stall detected", data->dev->name);
			stallguard_enable(data->dev, false);
			driver_trigger_cb(config->stepper_drivers[data->next_index], STEPPER_DRV_EVENT_STALL_DETECTED);
			break;
		default:
...
static int tmc50xx_stepper_move_to(const struct device *dev, const int32_t micro_steps)
{
	...
	struct tmc50xx_data *comm_data = comm_device->data;
	int err;
	...
	if (data->callback) {
		rampstat_work_reschedule(&comm_data->rampstat_callback_dwork);
	}
	return 0;
}
...
static int tmc50xx_init(const struct device *dev)
{
	...
	LOG_DBG("Num of steppers: %d %s", config->num_steppers, config->stepper_drivers[0]->name);
...
#define TMC50XX_MOTION_CONTROLLER_DEFINE(inst)                                                     \
	IF_ENABLED(CONFIG_STEPPER_ADI_TMC50XX_RAMP_GEN,						\
			(CHECK_RAMP_DT_DATA(DT_DRV_INST(inst))));   \
...
#define TMC50XX_DEFINE(inst)                                                                       \
	...
	BUILD_ASSERT(DT_INST_CHILD_NUM(inst) <= 2, "tmc50xx can drive two steppers at max");     b  \
